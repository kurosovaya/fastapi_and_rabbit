...
class PostgresStorage(Storage):
    def __init__(
        ...
        self.postgres_client: AsyncConnectionPool[AsyncConnection[DictRow]] = postgres_client
    ...
    async def get_event_id(self, idempotency_key: str) -> str:
        async with self.postgres_client.connection() as conn:
            row = await (await conn.execute("""select from events
    ...
    async def get_event(self, event_id: str):
        async with self.postgres_client.connection() as conn:
            ...
            """select * from events
            where _id = %(event_id)s""", {"event_id": event_id})).fetchone()
        return row

    async def get_unpublished_events(self, start_id: str | None = None):

        async with self.postgres_client.connection() as conn:
            batch = await (await conn.execute("""select * from events
            where published = false AND (%(start_id)s::text IS NULL OR _id > %(start_id)s)
            order by _id
            limit %(limit)s""", {"start_id": start_id, "limit": 500})).fetchall()
        return batch

    async def mark_event_as_published(self, id: str):
        async with self.postgres_client.connection() as conn:
            await conn.execute("""update events
            set published = true
            where _id = %s""", (id,))

    async def find_delivery(self, event_id: str, subscription_id: str):
        async with self.postgres_client.connection() as conn:
            return await (await conn.execute("""select * from deliveries
            where event_id = %s AND subscription_id = %s""", (event_id, subscription_id))).fetchone()

    async def create_delivery(self, dlv_id: str, event_id: str, subscription_id: str):
        async with self.postgres_client.connection() as conn:
            await conn.execute("""INSERT INTO deliveries
            ...
            (dlv_id, event_id, subscription_id, None, None, None, None, datetime.now(dt.UTC), None))
    ...
    async def update_delivery_status(
        ...
        async with self.postgres_client.connection() as conn:
            await conn.execute("""UPDATE deliveries
            SET status=%(status)s,
            ...
            """, {"attempt": attempt, "delivered_at": delivered_at,
                  "last_error": last_error, "next_attempt_at": next_attempt_at, "id": id,
                  "status": status})

    async def get_sink_settings(self) -> list[dict]:
        async with self.postgres_client.connection() as conn:
            rows = await (await conn.execute("""select client_id, accept_rate, delay_ms
            from sink_settings""")).fetchall()
        return [dict(row) for row in rows]

    async def set_sink_settings(self, client_id: str, accept_rate: int, delay_ms: int):
        async with self.postgres_client.connection() as conn:
            await conn.execute("""INSERT INTO sink_settings
